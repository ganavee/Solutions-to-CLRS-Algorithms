...
def newSeq(subSum, subTup, tup, k, arr, i):
    ...
    newSeq(subSum, subTup, newTup, k, arr, i+1)
    newSubTup = subTup + (ele, )
    del subTup
    #print(f"Second{i}: subSum = {subSum} subTup = {subTup} tup = {tup}, arr = {arr}")
    newSeq(subSum+ele, newSubTup, newTup, k, arr, i+1)
    return arr  
...
arr = []
seq = tuple(seq)
print(f"Subsequence of {seq} whose sum is equal to {k} is {newSeq(0, (), seq, k, arr, 1)}")
#stringSeq = input("Enter the String subsequence: ")
#string = ""
#print(f"Subsequences of {seq} is {subseq1(0, string, stringSeq, arr, k)}")
